...
探针还会输出 `E2E_MODEL_INIT_MS`、首次/稳定帧识别耗时，以及走子确认的
平均、P95 和最大耗时。它们是本机端到端基线，不是跨设备性能承诺。
...
## 2026-08-29 只读工作流延迟基线

证据编号：`E2E-WIN-LOCAL-20260829-LATENCY`

在 i7-14700KF、DPI 96、经典木色、红方在下的仓库自带离线棋盘上，模型
初始化 136 ms，首帧识别 115 ms，第二个稳定帧识别 120 ms。20 个连续、
逐步视觉确认的样本平均 380 ms、P95 416 ms、最大 447 ms，20/20 全部
`CONFIRMED`，协调器最终保持 `OBSERVING`。

这组数据低于只读工作流 800 ms 的 P95 预算；结合最终界面已测得的
1,454 ms 首条 PV，当前没有证据要求牺牲双稳帧、完整局面和合法性校验来
换取更短延迟。样本量仍有限，不能替代所有 DPI、动画速度和长期运行矩阵。
