...
本机开发镜像已经完成“启动即加载”的真实闭环：界面能读到稳定版 Pikafish，并同时保存最新 master 候选；两套 NNUE 在中文安装路径下分别绑定；精选本地 OBK 优先命中，ChessDB 补缺，单引擎分析持续输出深度、PV、分数和 NPS；窗口在查询与分析期间保持可交互，退出后没有残留应用或引擎进程。

这里的“最新稳定版”指 2026-08-28 能从 Pikafish 官方 Releases 验证的 `Pikafish-2026-01-02`。另外已从官方 `master@b97ef0f9` 本机构建 AVX-VNNI PGO 候选并配套 `master-net`，但它不替换稳定默认项。该选择兼顾稳定性、指令集适配和可追溯性，不作“任何机器、任何时限下绝对最强”的不可验证宣传。

## 本机运行配置

| 项目 | 配置 |
|---|---|
| 默认引擎 | Pikafish 2026-01-02 AVX-VNNI |
| 候选引擎 | Pikafish dev-20260826 `b97ef0f` AVX-VNNI PGO |
| NNUE | 默认与候选各用自己的官方匹配网络，不混用 |
| 线程 | 12 |
| 哈希 | 1,024 MiB |
| 候选线 | MultiPV 3 |
| 普通单步预算 | 固定时间 1,500 ms，可在界面修改 |
| 开局知识 | 622,175 行精选本地 OBK 优先；本地缺失时 ChessDB，超时 1,800 ms；再回退引擎 |
...
没有把社区里来源不清或没有明确再分发许可的所谓“最强 XQB/OBK”直接塞进软件。当前本地库来自用户提供的标准 SQLite OBK，经 `vscore>=3`、字段校验和同键同着确定性择优后缩成 622,175 行；ChessDB 继续负责本地缺失。详细证据见 [`opening-book-audit-2026-08-28.md`](opening-book-audit-2026-08-28.md)。
