...
| 只读识别稳定帧 | 120 ms；同轮首帧 115 ms，模型初始化 136 ms |
| 本地测试棋盘 20 步视觉确认 | 20/20；平均 380 ms，P95 416 ms，最大 447 ms |
...
- 只读连接在当前窗口识别出棋盘后自动完成无输入预校准；连接状态独立显示，
  不再被引擎 PV 文本覆盖。20 步确认 P95 已低于 800 ms 预算，因此保留
  100 ms 扫描间隔、双稳帧和合法性门槛。
