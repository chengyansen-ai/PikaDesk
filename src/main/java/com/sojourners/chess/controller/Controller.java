...
import com.sojourners.chess.ProductInfo;
...
import com.sojourners.chess.controller.component.GameTreePane;
import com.sojourners.chess.controller.component.GameTreeViewModel;
...
public class Controller implements EngineCallBack, LinkerCallBack, ChessManualCallBack {
    ...
    @FXML
    private BorderPane multiEnginePaneHost;
    private MultiEnginePane multiEnginePane;
    @FXML
    private BorderPane gameTreePaneHost;
    private GameTreePane gameTreePane;
    private GameTreeViewModel.Navigation gameTreeNavigation;
    ...
    public void changeTacticButtonClick(ActionEvent event) {
        if (robotRed.getValue() && redGo || robotBlack.getValue() && !redGo || robotAnalysis.getValue()) {
            ...
            engine.analysis(currentAnalysisFen(), currentAnalysisMoves(), tacticList);
        }
    }
    ...
    public void engineManageClick(ActionEvent e) {
        ...
        if (multiEnginePane != null) {
            multiEnginePane.reload(prop.getEngineConfigList());
        }
        // 如果引擎被卸载，则关闭
    ...
    private void engineGo() {
        ...
        engine.setHashSize(prop.getHashSize());
        engine.setAnalysisModel(robotAnalysis.getValue() ? Engine.AnalysisModel.INFINITE : prop.getAnalysisModel(), prop.getAnalysisValue());
        engine.analysis(currentAnalysisFen(), currentAnalysisMoves(), this.board.getBoard(), redGo);
    }
    ...
    public void exportImageMenuClick(ActionEvent e) {
        FileChooser fileChooser = new FileChooser();
        fileChooser.setInitialDirectory(new File(PathUtils.getJarPath()));
        fileChooser.setInitialFileName(ProductInfo.EXPORT_FILE_PREFIX + DateUtils.getDateTimeString(new Date()) + ".png");
    ...
    public void aboutClick(ActionEvent e) {
        DialogUtils.showInfoDialog("关于 " + ProductInfo.NAME, ProductInfo.aboutText());
    }

    @FXML
    public void upgradeClick(ActionEvent e) {
        SystemUtils.openBrowser(ProductInfo.UPSTREAM_RELEASES_URL);
    }

    @FXML
    public void instructionClick(ActionEvent e) {
        SystemUtils.openBrowser(ProductInfo.UPSTREAM_MANUAL_URL);
    }

    @FXML
    public void homeClick(ActionEvent e) {
        SystemUtils.openBrowser(ProductInfo.UPSTREAM_URL);
    }

    @FXML
    void localBookManageButtonClick(ActionEvent e) {
        if (App.openLocalBookDialog()) {
            OpenBookManager manager = OpenBookManager.getInstance();
            manager.setLocalOpenBooks();
            manager.diagnosticSummary().ifPresent(message ->
                    DialogUtils.showWarningDialog("开局库加载提示", message));
        }
    ...
    @FXML
    void bookBatchButtonClick(ActionEvent e) {
        App.openBookBatchDialog();
    }
    ...
    public void initialize() {
        ...
        // 初始化棋局
        newChessBoard(null);
        // 新分支树依赖已建立的初始棋局；传统棋谱表继续提供旧格式兼容。
        initGameTreePane(board.fenCode(true));
        // 加载引擎
        loadEngine(prop.getEngineName());
        // 多引擎分析工作区只在用户点击启动后创建进程
        initMultiEnginePane();
    }
    ...
    private void initMultiEnginePane() {
        multiEnginePane = new MultiEnginePane(
                prop.getEngineConfigList(),
                prop.getThreadNum(),
                prop.getHashSize(),
                this::currentMultiEngineRequest,
                this::prepareForMultiEngineAnalysis,
                this::restoreSingleEngineAfterMultiAnalysis);
        multiEnginePaneHost.setCenter(multiEnginePane);
    }

    private void initGameTreePane(String initialFen) {
        gameTreePane = new GameTreePane(initialFen, this::showGameTreeNode);
        gameTreePaneHost.setCenter(gameTreePane);
    }

    private void showGameTreeNode(GameTreeViewModel.Navigation navigation) {
        checkLinkMode();
        robotRed.setValue(false);
        redButton.setDisable(false);
        robotBlack.setValue(false);
        ...
        // Keep the legacy manual untouched. The selected tree node becomes an
        // explicit analysis context for both single- and multi-engine paths.
        gameTreeNavigation = navigation;
        board.browseChessRecord(navigation.positionFen(), List.of());
        board.setManualList(navigation.childMoves());
        redGo = "w".equals(navigation.positionFen().split("\\s+")[1]);
        refreshLineChart();
        listView.getItems().clear();
        infoShowLabel.setText("");
        doOpenBook();

        if (robotAnalysis.getValue()) engineGo();
        if (multiEnginePane != null) multiEnginePane.analyzeIfRunning();
    }

    private String currentAnalysisFen() {
        return gameTreeNavigation == null
                ? chessManualHandle.getFenCode() : gameTreeNavigation.positionFen();
    }

    private List<String> currentAnalysisMoves() {
        return gameTreeNavigation == null
                ? chessManualHandle.getMoveList() : List.of();
    }

    private MultiEngineAnalysisWorkspace.AnalysisRequest currentMultiEngineRequest() {
        MultiEngineAnalysisWorkspace.SearchMode mode = switch (prop.getAnalysisModel()) {
            case FIXED_STEPS -> MultiEngineAnalysisWorkspace.SearchMode.DEPTH;
            case FIXED_TIME -> MultiEngineAnalysisWorkspace.SearchMode.MOVETIME;
            case FIXED_NODES -> MultiEngineAnalysisWorkspace.SearchMode.NODES;
            case INFINITE -> MultiEngineAnalysisWorkspace.SearchMode.INFINITE;
        };
        long value = mode == MultiEngineAnalysisWorkspace.SearchMode.INFINITE
                ? 0 : prop.getAnalysisValue();
        return new MultiEngineAnalysisWorkspace.AnalysisRequest(
                currentAnalysisFen(),
                currentAnalysisMoves(),
                new MultiEngineAnalysisWorkspace.SearchLimit(mode, value));
    }

    private void prepareForMultiEngineAnalysis() {
        if (linkMode.getValue()) {
            stopGraphLink();
        }
        robotRed.setValue(false);
    ...
    private void restoreSingleEngineAfterMultiAnalysis() {
        if (engine == null) {
            loadEngine(prop.getEngineName());
        }
    }
    ...
    public void bestMove(String first, String second) {
        if (redGo && robotRed.getValue() || !redGo && robotBlack.getValue()) {
            ...
            if (linkMode.getValue()) {
                // The scanner observes the engine-board change and routes the
                // move through the fail-closed WindowsMoveCoordinator. Never
                // emit input directly from the engine callback.
                this.isThinking = false;
            }
        }
    }
            ...
            if (td.getValid()) {
                Platform.runLater(() -> {
                    ...
                    if (td.getPv() == 1) {
                        ...
                        if (gameTreePane != null && td.getDepth() != null) {
                            Integer centipawns = td.getMate() == null ? td.getScore() : null;
                            Integer mateIn = td.getMate() == null ? null : td.getScore();
                            gameTreePane.updateCurrentEvaluation(
                                    centipawns, mateIn, td.getDepth(), prop.getEngineName());
                        }
                    }
                });
            }
...
//        ExecutorsUtils.getInstance().close();
    ...
    @Override
    public ConnectionProfile configureConnection(ConnectionWizardState wizard) {
        if (Platform.isFxApplicationThread()) {
            return App.openLinkSetting(wizard);
        }
        FutureTask<ConnectionProfile> dialog = new FutureTask<>(
                () -> App.openLinkSetting(wizard));
        ...
        try {
            ...
            throw new IllegalStateException("无法打开连接配置向导", failure.getCause());
        }
    }
    ...
    public void connectionConfigurationFailed(String message) {
        Platform.runLater(() -> {
            ...
            if (message != null && !message.contains("已取消")) {
                DialogUtils.showWarningDialog("连接未启动", message);
            }
        });
    }
    ...
    public void browseChessRecord(String fenCode, List<String> moveList, boolean redGo, List<String> nextList) {
        ...
        gameTreeNavigation = null;
        if (gameTreePane != null) {
            gameTreePane.followLine(fenCode, moveList);
        }
        // 棋盘
