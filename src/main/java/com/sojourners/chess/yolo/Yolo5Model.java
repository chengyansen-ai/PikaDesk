...
import com.sojourners.chess.recognition.YoloRecognitionMapper;
...
public class Yolo5Model extends OnnxModel {
    ...
    @Override
    public RecognitionCandidate recognize(BufferedImage img, long inputBytes) {
        if (!inputWithinSafeLimits(img, inputBytes)) {
            return YoloRecognitionMapper.map(
                    img == null ? 0 : img.getWidth(), img == null ? 0 : img.getHeight(),
                    inputBytes, List.of(), getModelVersion());
        }
        try {
            List<DetectResult> results = predict(img);
            List<YoloRecognitionMapper.Detection> detections = results == null
                    ? List.of()
                    : results.stream()
                    .map(result -> new YoloRecognitionMapper.Detection(
                            result.label, result.rect.x, result.rect.y,
                            result.rect.width, result.rect.height, result.confidence))
                    .toList();
            return YoloRecognitionMapper.map(
                    img.getWidth(), img.getHeight(), inputBytes, detections, getModelVersion());
        } catch (Exception exception) {
            exception.printStackTrace();
            return YoloRecognitionMapper.map(
                    img.getWidth(), img.getHeight(), inputBytes, List.of(), getModelVersion());
        }
    }
    ...
    public java.awt.Rectangle findBoardPosition(BufferedImage img) {
        try {
            if (!inputWithinSafeLimits(img, estimatedInputBytes(img))) {
                return null;
            }
            // 图像宽高的缩放比例
